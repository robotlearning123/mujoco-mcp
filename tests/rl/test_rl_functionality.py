...
from mujoco_mcp.rl_integration import (
    ...
    example_training, ActionSpaceType, TaskType
)
...
class RLTestSuite:
    ...
    def test_rl_config(self):
        ...
        try:
            ...
            assert config.task_type == TaskType.REACHING
            assert config.max_episode_steps == 500
            assert config.action_space_type == ActionSpaceType.CONTINUOUS
            ...
            assert config2.action_space_type == ActionSpaceType.DISCRETE
    ...
    def test_environment_creation(self):
        ...
        try:
            ...
            config = RLConfig(robot_type="franka_panda", task_type=TaskType.REACHING)
            ...
            assert reaching_env.config.task_type == TaskType.REACHING

            balancing_env = create_balancing_env()
            assert balancing_env.config.task_type == TaskType.BALANCING

            walking_env = create_walking_env()
            assert walking_env.config.task_type == TaskType.WALKING
    ...
    def test_integration_completeness(self):
        ...
        try:
            ...
            assert env1.config.task_type == TaskType.REACHING
            assert env2.config.task_type == TaskType.BALANCING
            assert env3.config.task_type == TaskType.WALKING
